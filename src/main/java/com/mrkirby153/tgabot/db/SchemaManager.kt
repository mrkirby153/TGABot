    ...
    fun createTables() {
        ...
        dataSource.connection.use { con ->
            con.createStatement().executeUpdate(
                    "CREATE TABLE IF NOT EXISTS `categories` (id INT NOT NULL AUTO_INCREMENT, name VARCHAR(255), created_at TIMESTAMP NULL DEFAULT NULL, updated_at TIMESTAMP NULL DEFAULT NULL, PRIMARY KEY (id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4")
            con.createStatement().executeUpdate(
                    "CREATE TABLE IF NOT EXISTS `options` (id INT NOT NULL AUTO_INCREMENT, message_id VARCHAR(255), channel_id VARCHAR(255), category int NOT NULL, custom BOOLEAN DEFAULT FALSE, reaction VARCHAR(255) NOT NULL, created_at TIMESTAMP NULL DEFAULT NULL, updated_at TIMESTAMP NULL DEFAULT NULL, PRIMARY KEY (id), FOREIGN KEY (category) REFERENCES categories(id) ON DELETE CASCADE ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4")
            con.createStatement().executeUpdate(
